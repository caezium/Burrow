...
| **burrow_list_apps** | Before any uninstall, **always call this first**: each row carries `name`, `bundle_id`, `source`, `uninstall_name`, `path` and `size`, and `bundle_id` is the identifier `burrow_uninstall` wants — display names are not unique, and a row whose `bundle_id` is `"unknown"` cannot be targeted at all. `source: "Homebrew"` means the app is removed by `brew uninstall --cask --zap`, not the Trash. Also answers "what apps are installed?" On failure it returns an `error` and **no** `apps` key — never an empty list that reads as "nothing installed". | — |
