...
GROUPED_INTEGER = re.compile(r"^\s*\d{1,3}(?:,\d{3})+\s*$")
GROUP_SEPARATORS = {"de": ".", "es": ".", "pt-BR": ".", "ru": "\u00a0", "fr": "\u202f"}
...
class Slots(HTMLParser):
    ...
    def handle_data(self, data):
        if self.opaque_depth:
            return
        if GROUPED_INTEGER.fullmatch(data):
            self.events.append((self._pos(), "number", data))
            return
        if NOT_PROSE.match(data):
            return
...
def render(src, table, lang, page_name="index.html"):
    ...
    for start, end, kind, payload in slots(src):
        if kind in {"text", "number"}:
            translated = (payload.strip().replace(",", GROUP_SEPARATORS.get(lang, ","))
                          if kind == "number" else table.get(payload.strip()))
            if not translated:
                continue
    ...
    if lang != "en":
        ...
        page = re.sub(r'((?:href|src)=")(?![a-zA-Z][a-zA-Z0-9+.-]*:|/|#)([^"]*)"',
                      lambda m: rebase(m) + '"', page)
        # Fonts and other CSS assets use url(), so rebasing HTML attributes
        # alone leaves every translated page requesting nonexistent assets.
        page = re.sub(
            r'''(url\(\s*["']?)(?![a-zA-Z][a-zA-Z0-9+.-]*:|/|\#)([^\s)"']+)(["']?\s*\))''',
            lambda m: f'{m.group(1)}../{m.group(2)}{m.group(3)}', page)
        page = page.replace('<html lang="en">', f'<html lang="{lang}">')
        page = re.sub(r'<!-- Generated by scripts/site-release\.py[^>]*-->',
                      '<!-- Generated by scripts/site-i18n.py from the English source and docs/i18n catalogs. Do not edit by hand. -->',
    ...
    canonical = f"https://burrow.computer/{prefix}{path}"
