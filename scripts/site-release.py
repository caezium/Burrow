...
def page_html(*, title, desc, og_url, hero_h1, hero_p, hero_extra, main_html,
              extra_css, active, base="", head_extra=""):
    ...
    nav = (f'<nav class="nav">{navlink("./#tools", "Tools", "tools")}'
           f'{navlink("compare.html", "Compare", "compare")}'
           f'{navlink("blog/", "Blog", "blog")}'
           f'{navlink("install.html", "Install", "install")}'
...
def patch_index(html, rel, downloads=None, stars=None):
    ...
    # Download count only moves when --downloads asked for it, so `--check`
    # in CI never trips over a number that changes on its own.
    if downloads is not None:
        begin = html.find(DL_BEGIN)
        end = html.find(DL_END)
        if begin == -1 or end == -1:
            sys.exit("index.html: DL:BEGIN / DL:END markers not found")
        html = html[:begin] + render_downloads(downloads) + html[end + len(DL_END):]
    if stars is not None:
        begin = html.find(STARS_BEGIN)
        end = html.find(STARS_END)
        if begin == -1 or end == -1:
            sys.exit("index.html: STARS:BEGIN / STARS:END markers not found")
        html = (html[:begin] + STARS_BEGIN
                + f'<span class="nav-stars">{compact(stars)}</span>'
                + html[end:])
    return html
...
brew install --cask caezium/tap/burrow</div>
      ...
      <div class="inst-cell">
        ...
        <a class="inst-go" href="{REPO}/releases/latest" target="_blank" rel="noopener" data-burrow-analytics="download.install_page">Download for macOS {ARROW}</a>
      </div>
      <div class="inst-cell">
        <h2>Windows (beta)</h2>
        ...
        <a class="inst-go" href="{REPO}/releases/latest" target="_blank" rel="noopener" data-burrow-analytics="download.windows">Download the Windows preview {ARROW}</a>
      </div>
      <div class="inst-cell">
...
{INSTALL_JS}"""
    return page_html(
        ...
        hero_extra="", main_html=body, extra_css=INSTALL_CSS, active="install")
...
def render_compare(data):
    ...
    return page_html(
        ...
        hero_extra="", main_html=body, extra_css=COMPARE_CSS, active="compare")
...
def render_blog_post(post, cats, others):
    ...
    return page_html(
        ...
        main_html=main, extra_css=BLOG_CSS, active="blog", base="../",
        head_extra=f'\n<script type="application/ld+json">{ld}</script>')


def render_blog_index(data):
    ...
    return page_html(
        ...
        extra_css=BLOG_CSS, active="blog", base="../",
        head_extra=f'\n<script type="application/ld+json">{ld}</script>')
...
def main():
    ...
    downloads = fetch_download_total() if args.stats else None
    stars = fetch_stars() if args.stats else None
    ...
    targets = {
        INDEX: patch_index(INDEX.read_text(encoding="utf-8"), releases[0],
                           downloads, stars),
        INSTALL_HTML: render_install(),
        COMPARE_HTML: render_compare(compare),
        BLOG_INDEX: render_blog_index(blog),
    ...
    for post in blog_posts:
        targets[BLOG_DIR / f'{post["slug"]}.html'] = render_blog_post(
            post, blog["categories"], blog_posts)
