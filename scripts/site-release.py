...
Owns four things:
  ...
  5. The hero download count (DL:BEGIN/DL:END), but only under --downloads —
     it changes on its own, so --check must never depend on it.
...
Usage:
  ...
  python3 scripts/site-release.py --downloads   # also refresh the hero download count
...
def fetch_download_total():
    ...
    try:
        while True:
            url = (f"https://api.github.com/repos/caezium/Burrow/releases"
...
def patch_index(html, rel, downloads=None):
...
def main():
    ...
    ap.add_argument("--downloads", action="store_true",
                    help="also refresh the hero download count from the GitHub "
                         "releases API (needs network; left alone otherwise)")
    args = ap.parse_args()
    ...
    downloads = fetch_download_total() if args.downloads else None

    targets = {
        INDEX: patch_index(INDEX.read_text(encoding="utf-8"), releases[0], downloads),
