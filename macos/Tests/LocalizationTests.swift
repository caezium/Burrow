...
final class LocalizationTests: XCTestCase {
    ...
    ///
    /// This loops `AppLanguage.translated` rather than naming languages: a
    /// hardcoded list silently stops covering language ten, which is exactly
    /// how six tables once shipped with no assertions on them at all.
    ...
    /// others. This is also what stops English copy from being edited without
    /// the translations following: the key *is* the English string, so
    ...
    /// The folder can ship while the table inside it does not. Every other
    /// assertion here reads the checked-in source, so this is the only thing
    /// standing between a correct table and an app that renders English
    /// because the resource never got staged.
