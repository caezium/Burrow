...
class Client:
    ...
    def __init__(self, capabilities=None, declare_version=VERSION):
        ...
        # Register immediately after spawn, before any protocol work can fail.
    ...
    def send(self, method, params=None, raw=None, want_reply=True):
        ...
        if not want_reply:
            return None
        out = self.read_reply(method)
        if not out:
            raise RuntimeError(f"server closed the stream after {method}")
        return json.loads(out)

    def read_reply(self, method):
        deadline = time.monotonic() + REQUEST_TIMEOUT
        # This macOS harness polls the pipe itself. Calling readline after a
        # readiness check would still hang on a partial line with no newline.
        with selectors.DefaultSelector() as selector:
            ...
            while b"\n" not in self._read_buffer:
                ...
                if remaining <= 0 or not selector.select(remaining):
                    raise TimeoutError(
                        f"{method or 'raw request'} timed out waiting for a reply after {REQUEST_TIMEOUT:g}s")
                chunk = os.read(self.proc.stdout.fileno(), 65536)
                if not chunk:
                    reply, self._read_buffer = self._read_buffer, b""
                    return reply.decode("utf-8")
                self._read_buffer += chunk
        reply, _, self._read_buffer = self._read_buffer.partition(b"\n")
        return reply.decode("utf-8")
    ...
    def close(self, grace_period=10):
        if self._closed:
            return
        self._closed = True
        try:
        ...
        try:
            ...
            try:
                ...
                pass
            # kill() only sends the signal; wait() releases the child process.
            self.proc.wait()
        finally:
...
def main(argv=None):
    ...
    if not args:
        print(f"usage: {sys.argv[0]} <path-to-Burrow-binary>", file=sys.stderr)
        return 1
    BIN = args[0]
    PASSES.clear()
    FAILURES.clear()
    ...
    try:
        ...
        # This covers discovery, tools, and temporary legacy/elicitation clients,
        # including failures before a local variable received the new Client.
        for client in reversed(Client.active):
            try:
                client.close()
            except Exception as error:
                check("server cleanup", False, f"{type(error).__name__}: {error}")
        Client.active.clear()
