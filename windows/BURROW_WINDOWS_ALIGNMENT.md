...
- **Public source reads, fail closed.** The workflow fetches the public `caezium/burrow-cli` source snapshot over HTTPS without a read token or Git credential rewrite. Checkout does not persist credentials, and the published CLI has no private Cargo git dependencies. An invalid or missing gitlink, failed clone/checkout, or failed Cargo build stops the run. `scripts/tests/test_public_runtime_fetch.py` executes the workflow block against fake Git/Cargo commands to cover these failures and successful staging without credentials; `test_release_workflows.py` checks the credential boundaries.
